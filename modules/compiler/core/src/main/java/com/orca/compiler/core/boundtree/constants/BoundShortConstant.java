...
public final class BoundShortConstant extends BoundNumericConstant {

    private final short value;

    public BoundShortConstant(short value) {
        this.value = value;
    }
    ...
    public BoundNumericConstant negate() {
        return new BoundShortConstant((short) -value);
    }
    ...
    public BoundNumericConstant add(BoundNumericConstant other) {
        return switch (other) {
            case BoundByteConstant b ->
                new BoundShortConstant((short) (value + b.value()));
            case BoundShortConstant s ->
                new BoundShortConstant((short) (value + s.value()));
            case BoundIntConstant i ->
                new BoundIntConstant(value + i.value());
            case BoundLongConstant l ->
                new BoundLongConstant(value + l.value());
            case BoundFloatConstant f ->
                new BoundFloatConstant(value + f.value());
            case BoundDoubleConstant d ->
                new BoundDoubleConstant(value + d.value());
        };
    }
    ...
    public int compareTo(BoundNumericConstant other) {
        return switch (other) {
            case BoundByteConstant b ->
                Short.compare(value, b.value());
            case BoundShortConstant s ->
                Short.compare(value, s.value());
            case BoundIntConstant i ->
                Integer.compare(value, i.value());
            case BoundLongConstant l ->
                Long.compare(value, l.value());
            case BoundFloatConstant f ->
                Float.compare(value, f.value());
            case BoundDoubleConstant d ->
                Double.compare(value, d.value());
        };
    }
    ...
    public BoundConstant convertTo(LangType targetType) {
        return switch (targetType.getPrimitiveKind()) {
            ...
            case Short ->
                this;
            case Int ->
                new BoundIntConstant(value);
            case Long ->
                new BoundLongConstant(value);
            case Float ->
                new BoundFloatConstant(value);
            case Double ->
                new BoundDoubleConstant(value);
            case Bool, Char, String, None ->
                throw new IllegalArgumentException("Cannot convert short to " + targetType);
        };
    }
}

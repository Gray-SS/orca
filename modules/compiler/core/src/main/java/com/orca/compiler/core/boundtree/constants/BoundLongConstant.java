...
public final class BoundLongConstant extends BoundNumericConstant {
    ...
    public BoundNumericConstant add(BoundNumericConstant other) {
        return switch (other) {
            case BoundByteConstant b ->
                new BoundLongConstant(value + b.value());
            case BoundShortConstant s ->
                new BoundLongConstant(value + s.value());
            case BoundIntConstant i ->
                new BoundLongConstant(value + i.value());
    ...
    public int compareTo(BoundNumericConstant other) {
        return switch (other) {
            case BoundByteConstant b ->
                Long.compare(value, b.value());
            case BoundShortConstant s ->
                Long.compare(value, s.value());
            case BoundIntConstant i ->
                Long.compare(value, i.value());
            case BoundLongConstant l ->
    ...
    public BoundConstant convertTo(LangType targetType) {
        return switch (targetType.getPrimitiveKind()) {
            ...
            case Bool, Char, String, None ->
                throw new IllegalArgumentException("Cannot convert long to " + targetType);
        };
    }
